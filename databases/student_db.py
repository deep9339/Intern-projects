...
class student_db:
	...
	@classmethod
	def get_records_by_hall(cls, hall_nam):

		conn = sqlite.connect(file)
		c = conn.cursor()


		c.execute("SELECT oid, * FROM student_table WHERE hall_name = ?", (hall_nam, ))
		record = c.fetchall()
	...
	@classmethod
	def update_hall_name(cls, hall_nam, id_):

		conn = sqlite.connect(file)
		c = conn.cursor()

		c.execute("""UPDATE student_table SET hall_name = ?
					WHERE oid = ? """, (hall_nam,id_))
